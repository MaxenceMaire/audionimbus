...
```toml
[dependencies]
audionimbus-sys = { version = "4.8.2-rc.3", features = ["auto-install"] }
```
...
```toml
[dependencies]
audionimbus-sys = { version = "4.8.2-rc.3", features = ["auto-install", "fmod"] }
```
...
```toml
[dependencies]
audionimbus-sys = { version = "4.8.2-rc.3", features = ["auto-install", "wwise"] }
```
...
4. Set the `FMODSDK` environment variable to the path of the FMOD SDK installed on your system (e.g. `export FMODSDK="/path/to/FMOD"`).

5. Finally, add `audionimbus-sys` with the `fmod` feature enabled to your `Cargo.toml`:

```toml
[dependencies]
audionimbus-sys = { version = "4.8.2-rc.3", features = ["fmod"] }
```
...
```toml
[dependencies]
audionimbus-sys = { version = "4.8.2-rc.3", features = ["wwise"] }
```
